...
def evaluate(pred, gt):
    ...
    if TN + FP!=torch.tensor(0.0):
        Specificity = TN / (TN + FP)
    else:
        Specificity = torch.tensor(0.0)
    # Precision or positive predictive value
    if TP + FP!=torch.tensor(0.0):
         Precision = TP / (TP + FP)
    else:
        Precision=torch.tensor(0.0)
    # F1 score = Dice
    if Precision + Recall!=torch.tensor(0.0):
    ...
    return Precision, Recall, Specificity, F1, auc_roc, accuracy, IoU, DICE,  hd
